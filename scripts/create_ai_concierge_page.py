#!/usr/bin/env python3
"""Create the AI Concierge waitlist landing page on elmandrye.

Page is created as DRAFT (unpublished). Uses placeholder gradient cards
for agent photos — once the user generates the real photos, run
update_ai_concierge_photos.py to swap them in.

Re-running this script will fail with a handle collision (intentional —
don't accidentally overwrite the page after edits).
"""
...
def agent_card(agent):
    ...
    return f"""
      <div class="aic-agent">
        <div class="aic-agent-photo aic-photo-placeholder" id="aic-photo-{slug}"><span>{agent['name']}</span></div>
        <h3>{agent['name']}</h3>
        <p class="aic-role">{agent['role']}</p>
        <p class="aic-bio">{agent['bio']}</p>
      </div>
    """
...
<style>
  ...
  /* Hide the default page header title if the theme renders one above */
  ...
  .aic-photo-placeholder { display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #e4dccc 0%, #c9b78a 100%); color: #6b5b3a; font-size: 28px; font-weight: 700; letter-spacing: 0.5px; }
...
def main():
    ...
    print(f"Created: {page['id']}  /pages/{page['handle']}  published={page['isPublished']}")
    nid = page['id'].split('/')[-1]
    print(f"Admin URL: https://elmandrye.myshopify.com/admin/pages/{nid}")
